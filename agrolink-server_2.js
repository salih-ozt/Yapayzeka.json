...
const COMPRESSION_CONFIG = {
    image: { 
        quality: 75,           // 🚀 Hız için optimize edildi
        maxWidth: 1280, 
        maxHeight: 720,
    ...
    video: { 
        ...
        codec: 'libx264', 
        audioCodec: 'aac',
        audioBitrate: '128k',         // 🚀 Hız için optimize edildi
        quality: 23,                  // 🚀 CRF 23 = iyi kalite + hızlı
        preset: 'veryfast',           // 🚀 HIZLI PRESET
        tune: 'fastdecode',           // 🚀 Hızlı decode için optimize
        movflags: '+faststart',
        ...
        ultraFast: {
            preset: 'ultrafast',      // 🚀 EN HIZLI PRESET
            crf: 26,                  // 🚀 Hız için biraz düşük kalite
            maxWidth: 1280,           // 🚀 720p ultra hızlı mod
            maxHeight: 720,
            fps: 30,
            audioBitrate: '96k'       // 🚀 Düşük ses bitrate
        },
        // Büyük dosya eşiği (bu boyutun üstündekiler arka planda işlenir)
        backgroundProcessingThreshold: 50 * 1024 * 1024, // 🚀 50MB'a düşürüldü - daha erken arka plan
        // Maksimum video süresi (saniye)
        maxDuration: 300,             // 🚀 5 dakika max (hız için)
        // Küçük dosya eşiği (bu boyutun altındakiler direkt kopyalanır)
        skipProcessingThreshold: 10 * 1024 * 1024  // 🚀 10MB altı direkt kopyala
    },
    product: { 
        width: 1080, 
        height: 1080, 
...
// 🚀 HIZLANDIRILMIŞ RESİM SIKIŞTIIRMA (server_1.js entegrasyonu)
async function compressImage(inputPath, outputPath, options = {}) {
    try {
        const { width = 720, height = 720, quality = 60, maxWidth = 1280, maxHeight = 720 } = options;
        
        // 🎯 Dosya boyutunu kontrol et
        const stats = fssync.statSync(inputPath);
        const fileSizeMB = stats.size / (1024 * 1024);
        
        // 🚀 HIZLI MOD: Küçük dosyalar için hızlı işleme
        let useQuality = quality;
        let targetMaxWidth = maxWidth;
        let targetMaxHeight = maxHeight;
        
        if (fileSizeMB > 50) {
            // Büyük dosyalar: düşük kalite + küçük boyut
            useQuality = Math.min(quality, 50);
            targetMaxWidth = Math.min(maxWidth, 1280);
            targetMaxHeight = Math.min(maxHeight, 720);
            console.log(`🎯 Büyük dosya: ${fileSizeMB.toFixed(1)}MB → 720p (Q:${useQuality})`);
        ...
        // Küçük dosyalar: varsayılan ayarlar
        
        const image = sharp(inputPath, {
            failOnError: false,
            limitInputPixels: 268402689,  // 🚀 Standart limit - hız için
            sequentialRead: true
        });
        ...
        await image
            ...
            .webp({ 
                quality: useQuality,
                effort: 1  // 🚀 EN HIZLI işleme
            })
            .toFile(outputPath);
        
        await fs.unlink(inputPath).catch(() => {});
        
        console.log(`✅ Görsel: ${metadata.width}x${metadata.height} → ${targetWidth}x${targetHeight} (Q:${useQuality})`);
...
// Video dönüştürme - 🚀 HIZLANDIRILMIŞ FFmpeg İŞLEME (server_1.js entegrasyonu)
async function optimizeVideo(inputPath, outputPath, backgroundMode = false) {
    return new Promise(async (resolve, reject) => {
        ...
        console.log(`🎬 Video işleniyor: ${fileSizeMB.toFixed(1)}MB (${backgroundMode ? 'ARKA PLAN' : 'ÖN PLAN'})`);
        
        // 🚀 HIZLI MOD: Küçük videolar veya zaten optimize edilmiş videoları kopyala
        if (fileSizeMB < 10) {
            try {
                await fs.copyFile(inputPath, outputPath);
                await fs.unlink(inputPath).catch(() => {});
                const totalTime = (Date.now() - startTime) / 1000;
                console.log(`✅ Küçük video kopyalandı: ${fileSizeMB.toFixed(1)}MB (${totalTime.toFixed(1)}s)`);
                return resolve({ success: true, optimized: false, message: 'Küçük video direkt kopyalandı' });
            } catch (err) {
                return reject(err);
            }
        }
        ...
        ffmpeg.ffprobe(inputPath, async (err, metadata) => {
            if (err) {
                ...
                try {
                    await fs.copyFile(inputPath, outputPath);
                    await fs.unlink(inputPath).catch(() => {});
                    return resolve({ success: true, optimized: false });
                } catch (copyErr) {
            ...
            const originalCodec = videoStream.codec_name;
            const originalAudioCodec = audioStream ? audioStream.codec_name : null;
            
            console.log(`📊 Orijinal: ${originalWidth}x${originalHeight}, Codec: ${originalCodec}, Audio: ${originalAudioCodec}`);
            
            // 🚀 HIZLI KONTROL: Zaten optimize edilmiş videoyu kopyala
            const isAlreadyOptimized = originalCodec === 'h264' && 
                                       originalWidth <= COMPRESSION_CONFIG.video.maxWidth && 
                                       originalHeight <= COMPRESSION_CONFIG.video.maxHeight;
            
            if (isAlreadyOptimized && fileSizeMB < 50) {
                console.log('📋 Video zaten optimize, direkt kopyalanıyor...');
                try {
                    await fs.copyFile(inputPath, outputPath);
                    await fs.unlink(inputPath).catch(() => {});
                    const totalTime = (Date.now() - startTime) / 1000;
                    console.log(`✅ Optimize video kopyalandı: ${totalTime.toFixed(1)}s`);
                    return resolve({ 
                        success: true, 
                        optimized: false,
                        width: originalWidth,
            ...
            if (originalWidth > COMPRESSION_CONFIG.video.maxWidth || originalHeight > COMPRESSION_CONFIG.video.maxHeight) {
                const ratio = Math.min(
                    COMPRESSION_CONFIG.video.maxWidth / originalWidth,
                    COMPRESSION_CONFIG.video.maxHeight / originalHeight
                );
                targetWidth = Math.round(originalWidth * ratio);
                targetHeight = Math.round(originalHeight * ratio);
                
                // Çift sayıya yuvarla (ffmpeg gereksinimi)
                if (targetWidth % 2 !== 0) targetWidth += 1;
                if (targetHeight % 2 !== 0) targetHeight += 1;
            }
            
            // 🚀 HIZLI PRESET SEÇİMİ
            const useUltraFast = fileSizeMB > 100 || backgroundMode;
            const preset = useUltraFast ? 'ultrafast' : 'veryfast';
            const crf = useUltraFast ? COMPRESSION_CONFIG.video.ultraFast.crf : COMPRESSION_CONFIG.video.quality;
            
            const command = ffmpeg(inputPath);
            
            command.videoCodec(COMPRESSION_CONFIG.video.codec);
            command.size(`${targetWidth}x${targetHeight}`);
            command.outputOptions(['-preset', preset]);
            command.outputOptions(['-crf', crf.toString()]);
            command.outputOptions(['-threads', COMPRESSION_CONFIG.video.threads]);
            
            // FPS limitleme
            const fps = videoStream.r_frame_rate ? Math.min(eval(videoStream.r_frame_rate), COMPRESSION_CONFIG.video.fps) : COMPRESSION_CONFIG.video.fps;
            command.fps(fps);
            
            // 🚀 SES OPTİMİZASYONU
            if (originalAudioCodec && originalAudioCodec.toLowerCase() === 'aac') {
                command.audioCodec('copy');
            } else if (audioStream) {
                command.audioCodec(COMPRESSION_CONFIG.video.audioCodec);
                command.audioBitrate(COMPRESSION_CONFIG.video.audioBitrate);
            } else {
                command.noAudio();
            }
            
            command.outputOptions(['-movflags', COMPRESSION_CONFIG.video.movflags]);
            ...
            command.on('start', (commandLine) => {
                console.log(`🎬 FFmpeg başlatıldı: ${targetWidth}x${targetHeight}, Preset: ${preset}, CRF: ${crf}`);
            })
            .on('progress', (progress) => {
                if (progress.percent && progress.percent % 25 < 5) {
                    console.log(`⏳ Video ilerleme: ${Math.round(progress.percent)}%`);
                }
            })
            .on('end', async () => {
                const totalTime = (Date.now() - startTime) / 1000;
                ...
                try {
                    ...
                    console.log(`✅ Video optimize: ${originalWidth}x${originalHeight} → ${targetWidth}x${targetHeight} | ${fileSizeMB.toFixed(1)}MB → ${outputSizeMB.toFixed(1)}MB (${compressionRatio}% sıkıştırma, ${totalTime.toFixed(1)}s)`);
                } catch (e) {
                    console.log(`✅ Video optimize tamamlandı: ${targetWidth}x${targetHeight} (${totalTime.toFixed(1)}s)`);
                }
            ...
            .on('error', async (err) => {
                console.error('❌ Video optimize hatası:', err.message);
                try {
                    await fs.copyFile(inputPath, outputPath);
                    console.log('⚠️ Video optimize başarısız, orijinal dosya kullanılıyor');
                    await fs.unlink(inputPath).catch(() => {});
                    resolve({ 
                        success: true, 
                        optimized: false,
                    ...
                    console.error('❌ Dosya kopyalama hatası:', copyErr);
                    reject(err);
                }
            })
            .save(outputPath);
