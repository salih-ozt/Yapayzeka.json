...
app.get('/api/partnership/action/:id/:action', async (req, res) => {
    try {
        ...
        if (!['approve', 'reject'].includes(action)) {
            return res.status(400).send(actionPageShell('Hata', '<p style="color:#ef4444">Geçersiz işlem.</p>'));
        }

        const application = await dbGet(`SELECT * FROM partnership_applications WHERE id = $1`, [id]);
        if (!application) {
            return res.status(404).send(actionPageShell('Bulunamadı', `
        ...
        if (application.status !== 'pending') {
            ...
            return res.send(actionPageShell('Zaten İşlendi', `
              <div class="success-wrap">
        ...
        res.send(actionPageShell(isApprove ? 'Başvuruyu Onayla' : 'Başvuruyu Reddet', formBody));

    } catch (error) {
        console.error('Partnership action GET hatası:', error);
        res.status(500).send(actionPageShell('Hata', '<p style="color:#ef4444">Sunucu hatası oluştu.</p>'));
    }
});
...
app.post('/api/partnership/action/:id/:action', async (req, res) => {
    try {
        ...
        if (!customMessage) {
            return res.status(400).send(actionPageShell('Hata', '<p style="color:#ef4444">Mesaj boş olamaz.</p>'));
        }

        const application = await dbGet(`SELECT * FROM partnership_applications WHERE id = $1`, [id]);
        if (!application) return res.status(404).send(actionPageShell('Hata', '<p style="color:#ef4444">Başvuru bulunamadı.</p>'));
        if (application.status !== 'pending') return res.send(actionPageShell('Zaten İşlendi', `
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#0f172a;border-radius:24px;overflow:hidden;border:1px solid rgba(255,255,255,0.07);box-shadow:0 24px 64px rgba(0,0,0,0.5)">
      ...
      <tr><td style="background:rgba(0,0,0,0.2);padding:20px 44px;border-top:1px solid rgba(255,255,255,0.05)">
        ...
        res.send(actionPageShell(isApproved ? 'Onaylandı' : 'Reddedildi', resultBody));
