...
// 🔒 DÜZELTMESİ: cf-connecting-ip / x-real-ip / x-forwarded-for header'ları
// Cloudflare veya Nginx tarafından güvenilir şekilde SET EDİLMİYORSA (yani
// doğrudan istemciden geçiyorsa) rastgele bir değerle sınırsız rate-limit
// bypass edilebilir. Bu fonksiyon sadece uygulama trust proxy ile uyumlu,
// reverse proxy'nin bu header'ları HER ZAMAN kendi tespit ettiği gerçek
// bağlantı IP'siyle üzerine yazdığı (overwrite ettiği) varsayımıyla güvenlidir.
// Nginx örneği:
//   proxy_set_header X-Real-IP $remote_addr;
//   proxy_set_header X-Forwarded-For $remote_addr;   # $proxy_add_x_forwarded_for KULLANMAYIN
// Aksi halde istemci bu header'ları serbestçe göndererek rate limit'i atlatabilir.
...
async function authenticateToken(req, res, next) {
    ...
    try {
        ...
        // 🔒 3b. 2FA/OTP bekleyen geçici token'lar (tempToken) burada ASLA kabul edilmez.
        // tempToken aynı JWT_SECRET ile imzalanıyor ama pending2FA:true taşıyor —
        // 2FA tamamlanmadan tam yetkili erişim vermemesi için burada reddediyoruz.
...
// 🔒 DÜZELTMESİ: Eskiden "Authorization: Bearer <herhangi bir değer>" reCAPTCHA'yı
// doğrulanmadan atlatabiliyordu. Artık Bearer token varsa GERÇEKTEN jwt.verify ile
// doğrulanır — sadece geçerli, mevcut bir access token reCAPTCHA'yı atlar.
async function recaptchaMiddleware(req, res, next) {
    ...
    // Bearer token varsa gerçekten doğrula — sadece geçerli bir access token reCAPTCHA'yı atlar
    const authHeader = req.headers['authorization'];
    if (authHeader && authHeader.startsWith('Bearer ')) {
        const bearerToken = authHeader.slice(7);
        try {
            jwt.verify(bearerToken, JWT_SECRET, { algorithms: ['HS256'] });
            console.log('[reCAPTCHA] Geçerli Bearer token ile istek — reCAPTCHA atlanıyor');
            return next();
        } catch (e) {
            console.warn('[reCAPTCHA] Geçersiz/sahte Bearer token — reCAPTCHA bypass reddedildi');
            // Geçersiz token → normal reCAPTCHA akışına devam et, atlamaya izin verme
        }
    }
...
app.put('/api/users/profile', authenticateToken, (req, res, next) => {
    ...
    try {
        ...
        // 🔒 DÜZELTMESİ: string olmayan gövde (obje/array) gönderilirse .substring çökmesin
        if (name !== undefined) { updates.push(`name = $${paramIdx++}`); params.push(String(name).substring(0, 50)); }
        if (bio !== undefined) { updates.push(`bio = $${paramIdx++}`); params.push(String(bio).substring(0, 300)); }
        if (location !== undefined) { updates.push(`location = $${paramIdx++}`); params.push(String(location).substring(0, 100)); }
        if (website !== undefined) { updates.push(`website = $${paramIdx++}`); params.push(String(website).substring(0, 200)); }
